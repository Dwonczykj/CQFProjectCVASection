...
cdef class SobolNumbers:
    ...
    def ProcessSobolInitialisationNumbers(self):
        ...
        def ProcessDirections(s):
            st = s.split(' ')
            return np.append(np.array([int(float(a)) for a in st]),np.zeros(shape=(self.b-len(st))))
        SobolInitData['v_i'] = SobolInitData['m_i'].map(ProcessDirections)
        Z = SobolInitData['Degree'][self.MAXDIM-1]
        tv = SobolInitData['v_i'].values[0:self.MAXDIM]
        tv[0][0:self.b] = 1
        tvalt = tv#np.array(list(map(lambda x: np.array(list(x)), zip(*tv))))
        # initV = np.append([0],tvalt[0:Z])
        self.mdeg=SobolInitData['Degree'][0:self.MAXDIM].values
        self.initP=SobolInitData['iP'][0:self.MAXDIM].values
        # return initV
        return tvalt[0:Z]
